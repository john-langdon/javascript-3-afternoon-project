...
//Code Here
let postTaxPrices = prices.map(function(el) {
  return el * 1.07
})
...
//Code Here
let totalPopulation = populations.reduce(function(total, el) {
  return total + el;
})
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(el, i) {
  if (el['CP'] > 200) {
    return el
  }
})
...
let orderTotals = orders.map(function(el) {
  return el['price'] + el['price'] * el['tax']
})
...
let bobsTickets = purchases.filter(function(el) {
  return el.owner === 'Bob'
})


// console.log(bobsTickets)

let bobsTotal = bobsTickets.reduce(function(total, el) {
